...
gulp.task('replaceParams', ['compileTS'], function () {
	...
	console.log('org.weather.ui::gulpfile.js:: process.argv = ' + process.argv);
	console.log('org.weather.ui::gulpfile.js:: util.env.buildName = ' + util.env.buildName);
...
gulp.task("copyBowerLibs", ['bowerInstall'], function () {
    return bowerFiles().pipe(gulp.dest(output + "/bower_components"));
});
...
gulp.task('watch-dev', ['compileTS', 'copyAssets', 'copyImages', 'compileSass', 'server-start'], function (done) {
    gulp.watch(sassFiles, ['compileSass']);
   // gulp.watch(tsScriptFiles, ['compileTS']);
    gulp.watch(htmlFiles, ['copyHtml']);
    return done();
